...
def main():
    ...
    try:
        ...
        # Convert mask to binary (0-255) and ensure it is in "L" mode (grayscale)
        mask_binary = mask_resized.point(lambda p: p * 255).convert("L")

        # Create a green background image
        green_background = Image.new("RGB", image.size, (0, 255, 0))  # RGB green

        # Composite the original image and the green background
        # The mask acts as an alpha-like layer for blending
        composite_image = Image.composite(image, green_background, mask_binary)
